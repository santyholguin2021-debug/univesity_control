...
public class EnrollmentRepository implements CsvRepository<Enrollment> {
    ...
    private Enrollment parseEnrollment(String line) {
        ...
        enrollment.setStudentId(parts[1].isEmpty() ? null : parts[1]);
        enrollment.setCourseId(parts[2].isEmpty() ? null : parts[2]);
        return enrollment;
    }
    ...
    private String toCSV(Enrollment enrollment) {
        return String.join(",",
                enrollment.getId(),
                enrollment.getStudentId() == null ? "" : enrollment.getStudentId(),
